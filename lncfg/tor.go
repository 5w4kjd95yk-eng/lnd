...
type Tor struct {
	...
	SkipProxyForClearNetTargets bool     `long:"skip-proxy-for-clearnet-targets" description:"Route clearnet connections outside Tor, either directly or through the generic SOCKS5 proxy."`
	NoProxyTargets              []string `long:"no-proxy-target" description:"A host, *.domain zone, IP address, or CIDR that bypasses Tor. Can be repeated"`
